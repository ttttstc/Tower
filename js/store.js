// store.js — TowerProject 数据中心
// 单页静态站点共享。模块系统：原生 ES Module。
// 用法：import { Store, LAYERS, STATUSES } from './js/store.js';

export const LAYERS = [
  { key: 'vision',         name: '愿景层',     en: 'VISION' },
  { key: 'design',         name: '设计层',     en: 'DESIGN' },
  { key: 'coordination',   name: '协同层',     en: 'COORDINATION' },
  { key: 'implementation', name: '实现层',     en: 'IMPLEMENTATION' },
  { key: 'infrastructure', name: '基础设施层', en: 'INFRASTRUCTURE' },
];

export const LAYER_KEYS = LAYERS.map(l => l.key);

export const STATUSES = [
  { key: 'done',        name: '已完成', color: '#7fc060' },
  { key: 'in_progress', name: '进行中', color: '#f0a040' },
  { key: 'overdue',     name: '延期',   color: '#ffd040' },
  { key: 'blocked',     name: '阻塞',   color: '#ff5040' },
  { key: 'not_started', name: '待启动', color: '#9a8862' },
];

const STORAGE_KEY = 'tower.project.v1';
const SNAPSHOT_KEY_PREFIX = 'tower.snapshot.';
const LAST_OPEN_KEY = 'tower.lastOpenDate';
const JSON_FILE_TYPE = {
  description: 'Tower Project',
  accept: { 'application/json': ['.json'] },
};
const STATUS_PROGRESS = {
  done: 100,
  in_progress: 60,
  overdue: 40,
  blocked: 25,
  not_started: 0,
};

function uuid() {
  return 'i_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 9);
}

function nowIso() {
  return new Date().toISOString();
}

function todayStr() {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

function uniqStrings(values) {
  return Array.from(new Set((values || []).map(v => String(v || '').trim()).filter(Boolean)));
}

function clampProgress(value, status = 'not_started') {
  if (value === '' || value == null || Number.isNaN(Number(value))) return STATUS_PROGRESS[status] ?? 0;
  return Math.max(0, Math.min(100, Math.round(Number(value))));
}

function normalizeItem(raw = {}) {
  const id = String(raw.id || uuid());
  const risk = !!raw.risk;
  const layer = LAYER_KEYS.includes(raw.layer) ? raw.layer : 'implementation';
  const status = STATUSES.some(s => s.key === raw.status) ? raw.status : 'not_started';
  const progress = clampProgress(raw.progress, status);
  const effort = Math.max(1, Math.min(20, parseInt(raw.effort, 10) || 1));
  const createdAt = raw.createdAt || nowIso();
  const updatedAt = raw.updatedAt || createdAt;
  const dependencies = Array.isArray(raw.dependencies)
    ? Array.from(new Set(raw.dependencies.map(d => String(d)).filter(Boolean))).filter(d => d !== id)
    : [];
  return {
    id,
    title: String(raw.title || '新事项').trim() || '新事项',
    layer,
    status: risk ? 'blocked' : status,
    progress: risk ? Math.max(progress, STATUS_PROGRESS.blocked) : progress,
    effort,
    dependencies,
    assignee: String(raw.assignee || '').trim(),
    risk,
    notes: String(raw.notes || ''),
    createdAt,
    updatedAt,
  };
}

function normalizeProject(raw) {
  if (!raw || !Array.isArray(raw.items)) throw new Error('无效的 tower.json');
  const items = raw.items.map(normalizeItem);
  return {
    version: '1.0',
    name: String(raw.name || 'Tower 项目').trim() || 'Tower 项目',
    goal: String(raw.goal || '').trim(),
    acceptanceCriteria: String(raw.acceptanceCriteria || '').trim(),
    owners: uniqStrings([...(raw.owners || []), ...items.map(i => i.assignee)]),
    items,
  };
}

// ========== Demo seed data ==========
function seed() {
  const owners = ['PM', 'Arch', 'Dev-A', 'Dev-B', 'Dev-C', 'QA'];
  const T = [
    // vision
    ['vision', '北极星指标', 'done', 2, 'PM'],
    ['vision', '商业化路径', 'in_progress', 3, 'PM'],
    ['vision', '里程碑规划', 'done', 2, 'PM'],
    ['vision', '相关方对齐', 'in_progress', 2, 'PM'],
    // design
    ['design', '应县木塔视觉', 'done', 5, 'Arch'],
    ['design', '交互规范', 'in_progress', 4, 'Arch'],
    ['design', '组件库', 'in_progress', 6, 'Dev-B'],
    ['design', '色彩 token', 'done', 1, 'Arch'],
    ['design', '字体方案', 'done', 1, 'Arch'],
    // coordination
    ['coordination', '工作流引擎', 'in_progress', 5, 'Dev-A'],
    ['coordination', '权限模型', 'in_progress', 4, 'Dev-A'],
    ['coordination', 'Webhook 对接', 'not_started', 3, 'Dev-C'],
    ['coordination', '通知服务', 'blocked', 4, 'Dev-C'],
    ['coordination', '审计日志', 'not_started', 2, 'Dev-C'],
    // implementation
    ['implementation', 'API 网关', 'in_progress', 5, 'Dev-A'],
    ['implementation', '数据流引擎', 'in_progress', 6, 'Dev-A'],
    ['implementation', '触发链', 'overdue', 3, 'Dev-B'],
    ['implementation', '前端框架', 'done', 4, 'Dev-B'],
    ['implementation', 'E2E 测试', 'not_started', 4, 'QA'],
    ['implementation', '插件机制', 'not_started', 5, 'Dev-B'],
    // infrastructure
    ['infrastructure', 'K8s 集群', 'done', 5, 'Dev-C'],
    ['infrastructure', 'CI 镜像', 'done', 2, 'Dev-C'],
    ['infrastructure', '对象存储', 'done', 2, 'Dev-C'],
    ['infrastructure', 'CDN 部署', 'done', 1, 'Dev-C'],
    ['infrastructure', '监控告警', 'in_progress', 3, 'Dev-C'],
    ['infrastructure', '容灾演练', 'not_started', 3, 'Dev-C'],
  ];
  const items = T.map(([layer, title, status, effort, assignee]) => ({
    id: uuid(),
    title,
    layer,
    status,
    progress: STATUS_PROGRESS[status] ?? 0,
    effort,
    dependencies: [],
    assignee,
    risk: status === 'blocked',
    notes: '',
    createdAt: nowIso(),
    updatedAt: nowIso(),
  }));
  // 增加几条跨层依赖示例
  const find = (t) => items.find(x => x.title === t)?.id;
  const link = (from, to) => {
    const f = find(from), t = find(to);
    if (f && t) items.find(x => x.id === f).dependencies.push(t);
  };
  link('数据流引擎', 'K8s 集群');
  link('API 网关', '权限模型');
  link('工作流引擎', 'API 网关');
  link('交互规范', '应县木塔视觉');

  return {
    version: '1.0',
    name: 'Cloud-Native DevOps Platform',
    goal: '打造稳定高效安全易用的云原生 DevOps 平台',
    acceptanceCriteria: '覆盖需求、设计、协同、实现与基础设施全流程',
    owners,
    items,
  };
}

// ========== Store ==========
function loadRaw() {
  try {
    const s = localStorage.getItem(STORAGE_KEY);
    if (!s) return null;
    return JSON.parse(s);
  } catch { return null; }
}

function saveRaw(p) {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(p));
}

class StoreImpl {
  constructor() {
    this._listeners = new Set();
    this._fileHandle = null;
    this._fileName = '';
    let p = loadRaw();
    try {
      this._project = p ? normalizeProject(p) : normalizeProject(seed());
    } catch {
      this._project = normalizeProject(seed());
    }
    this._save();
    this._maybeSnapshot();
  }

  // ---------- subscribe ----------
  subscribe(fn) { this._listeners.add(fn); return () => this._listeners.delete(fn); }
  _emit(evt) { this._listeners.forEach(fn => fn(evt, this._project)); }

  // ---------- read ----------
  get project() { return this._project; }
  get items() { return this._project.items; }
  itemsByLayer(key) { return this._project.items.filter(i => i.layer === key); }
  getItem(id) { return this._project.items.find(i => i.id === id); }
  fileSystemSupported() {
    return typeof window !== 'undefined' && ('showOpenFilePicker' in window || 'showSaveFilePicker' in window);
  }
  boundFileName() { return this._fileName; }
  isBoundToFile() { return !!this._fileHandle; }

  // 楼段进度（已完成 effort / 总 effort）
  layerProgress(key) {
    const arr = this.itemsByLayer(key);
    const total = arr.reduce((s, i) => s + (i.effort || 0), 0);
    const done  = arr.reduce((s, i) => s + (i.status === 'done' ? (i.effort || 0) : 0), 0);
    return { total, done, pct: total ? Math.round(done * 100 / total) : 0, count: arr.length };
  }
  overallProgress() {
    const total = this._project.items.reduce((s, i) => s + (i.effort || 0), 0);
    const done  = this._project.items.reduce((s, i) => s + (i.status === 'done' ? (i.effort || 0) : 0), 0);
    return { total, done, pct: total ? Math.round(done * 100 / total) : 0, count: this._project.items.length, doneCount: this._project.items.filter(i => i.status === 'done').length };
  }

  // ---------- mutate ----------
  _save() { saveRaw(this._project); }
  _setProject(project, evt = { type: 'reset' }) {
    this._project = normalizeProject(project);
    this._save();
    this._emit(evt);
    return this._project;
  }

  setProjectMeta({ name, goal, acceptanceCriteria }) {
    if (name !== undefined) this._project.name = name;
    if (goal !== undefined) this._project.goal = goal;
    if (acceptanceCriteria !== undefined) this._project.acceptanceCriteria = acceptanceCriteria;
    this._save();
    this._emit({ type: 'meta' });
  }

  addOwner(name) {
    if (!name) return;
    if (!this._project.owners.includes(name)) {
      this._project.owners.push(name);
      this._save();
      this._emit({ type: 'owner-add', name });
    }
  }

  addItem(partial) {
    const it = {
      id: uuid(),
      title: partial.title || '新事项',
      layer: partial.layer || 'implementation',
      status: partial.status || 'not_started',
      progress: clampProgress(partial.progress, partial.status || 'not_started'),
      effort: Math.max(1, Math.min(20, parseInt(partial.effort) || 1)),
      dependencies: Array.isArray(partial.dependencies) ? partial.dependencies.slice() : [],
      assignee: partial.assignee || '',
      risk: !!partial.risk,
      notes: partial.notes || '',
      createdAt: nowIso(),
      updatedAt: nowIso(),
    };
    if (it.risk) {
      it.status = 'blocked';
      it.progress = Math.max(it.progress, STATUS_PROGRESS.blocked);
    }
    this._project.items.push(it);
    if (it.assignee) this.addOwner(it.assignee);
    this._save();
    this._emit({ type: 'item-add', id: it.id, layers: [it.layer] });
    return it;
  }

  updateItem(id, patch) {
    const it = this.getItem(id);
    if (!it) return null;
    const oldLayer = it.layer;
    const layers = new Set();
    Object.keys(patch).forEach(k => {
      if (k === 'id' || k === 'createdAt') return;
      if (k === 'effort') it[k] = Math.max(1, Math.min(20, parseInt(patch[k]) || 1));
      else if (k === 'progress') it[k] = clampProgress(patch[k], patch.status || it.status);
      else it[k] = patch[k];
    });
    if (it.risk) {
      it.status = 'blocked';
      it.progress = Math.max(clampProgress(it.progress, it.status), STATUS_PROGRESS.blocked);
    } else {
      it.progress = clampProgress(it.progress, it.status);
    }
    if (it.dependencies) it.dependencies = (it.dependencies || []).filter(d => d !== id);
    it.updatedAt = nowIso();
    if (it.layer !== oldLayer) { layers.add(oldLayer); layers.add(it.layer); }
    else layers.add(it.layer);
    if (it.assignee) this.addOwner(it.assignee);
    this._save();
    this._emit({ type: 'item-update', id, layers: [...layers] });
    return it;
  }

  removeItem(id) {
    const idx = this._project.items.findIndex(i => i.id === id);
    if (idx < 0) return;
    const layer = this._project.items[idx].layer;
    this._project.items.splice(idx, 1);
    // 清掉别人对它的依赖
    this._project.items.forEach(i => { i.dependencies = (i.dependencies || []).filter(d => d !== id); });
    this._save();
    this._emit({ type: 'item-remove', id, layers: [layer] });
  }

  bulkAddItems(items) {
    const created = [];
    items.forEach(p => { created.push(this.addItem(p)); });
    return created;
  }

  // ---------- 依赖环检测 ----------
  // 给定 itemId，候选 depId，是否会形成环（不允许）
  wouldCycle(itemId, depId) {
    if (itemId === depId) return true;
    // 从 depId 出发，沿 dependencies 走，能不能回到 itemId
    const visited = new Set();
    const stack = [depId];
    while (stack.length) {
      const cur = stack.pop();
      if (cur === itemId) return true;
      if (visited.has(cur)) continue;
      visited.add(cur);
      const it = this.getItem(cur);
      if (it) (it.dependencies || []).forEach(d => stack.push(d));
    }
    return false;
  }

  // ---------- 快照 / 日报 ----------
  _maybeSnapshot() {
    const today = todayStr();
    const last = localStorage.getItem(LAST_OPEN_KEY);
    if (last !== today) {
      // 把"今天打开前"的状态作为昨日快照
      const prev = last;
      if (prev) {
        const snapKey = SNAPSHOT_KEY_PREFIX + prev;
        if (!localStorage.getItem(snapKey)) {
          localStorage.setItem(snapKey, JSON.stringify(this._project));
        }
      }
      localStorage.setItem(LAST_OPEN_KEY, today);
    }
  }

  // 取最近一次的快照（昨日或更早）
  latestSnapshot() {
    const keys = Object.keys(localStorage)
      .filter(k => k.startsWith(SNAPSHOT_KEY_PREFIX))
      .sort();
    if (!keys.length) return null;
    try { return { date: keys[keys.length - 1].slice(SNAPSHOT_KEY_PREFIX.length), data: JSON.parse(localStorage.getItem(keys[keys.length - 1])) }; }
    catch { return null; }
  }

  // 生成日报 markdown（对比最近快照与当前）
  buildDailyReport() {
    const snap = this.latestSnapshot();
    const cur = this._project;
    const lines = [];
    lines.push(`# ${cur.name} · 今日日报`);
    lines.push('');
    lines.push(`> 生成于 ${new Date().toLocaleString('zh-CN')}`);
    lines.push('');
    if (!snap) {
      lines.push('（无历史快照可对比，仅展示当前状态）');
      lines.push('');
    } else {
      lines.push(`对比快照：${snap.date}`);
      lines.push('');
      const prevMap = new Map(snap.data.items.map(i => [i.id, i]));
      const curMap = new Map(cur.items.map(i => [i.id, i]));

      const added = cur.items.filter(i => !prevMap.has(i.id));
      const removed = snap.data.items.filter(i => !curMap.has(i.id));
      const changed = cur.items.filter(i => prevMap.has(i.id) && prevMap.get(i.id).status !== i.status);

      lines.push(`## 新增方块（${added.length}）`);
      added.forEach(i => lines.push(`- [${LAYERS.find(l=>l.key===i.layer)?.name}] ${i.title}（${i.assignee || '未分配'}）`));
      if (!added.length) lines.push('- 无');
      lines.push('');

      lines.push(`## 状态变化（${changed.length}）`);
      changed.forEach(i => {
        const prev = prevMap.get(i.id);
        lines.push(`- [${LAYERS.find(l=>l.key===i.layer)?.name}] ${i.title}: ${labelOf(prev.status)} → **${labelOf(i.status)}**`);
      });
      if (!changed.length) lines.push('- 无');
      lines.push('');

      lines.push(`## 已删除（${removed.length}）`);
      removed.forEach(i => lines.push(`- [${LAYERS.find(l=>l.key===i.layer)?.name}] ${i.title}`));
      if (!removed.length) lines.push('- 无');
      lines.push('');

      // 楼段封顶
      const sealed = LAYERS.filter(L => {
        const arr = cur.items.filter(i => i.layer === L.key);
        const wasSealed = snap.data.items.filter(i => i.layer === L.key).every(i => i.status === 'done');
        const isSealed  = arr.length > 0 && arr.every(i => i.status === 'done');
        return isSealed && !wasSealed;
      });
      lines.push(`## 新封顶楼段（${sealed.length}）`);
      sealed.forEach(L => lines.push(`- ${L.name} 🎉`));
      if (!sealed.length) lines.push('- 无');
      lines.push('');
    }
    // 当前总览
    const ov = this.overallProgress();
    lines.push(`## 当前总览`);
    lines.push(`- 整体进度 ${ov.pct}%（${ov.doneCount}/${ov.count} 项已完成）`);
    LAYERS.forEach(L => {
      const lp = this.layerProgress(L.key);
      lines.push(`- ${L.name}：${lp.pct}%（${lp.count} 项）`);
    });
    return lines.join('\n');
  }

  // ---------- 全量重置（debug） ----------
  resetToSeed() {
    this._setProject(seed(), { type: 'reset' });
  }

  // ---------- 导出 ----------
  exportJson() { return JSON.stringify(this._project, null, 2); }
  downloadJson(filename = '') {
    const safeName = filename || this._fileName || 'tower.json';
    const blob = new Blob([this.exportJson()], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = safeName;
    a.click();
    URL.revokeObjectURL(url);
    return { kind: 'download', name: safeName };
  }
  importJson(str, meta = {}) {
    const p = JSON.parse(str);
    this._fileHandle = meta.handle || null;
    this._fileName = meta.fileName || (meta.handle && meta.handle.name) || '';
    this._setProject(p, { type: 'reset' });
  }
  async openProjectFile() {
    if (typeof window === 'undefined' || !('showOpenFilePicker' in window)) {
      throw new Error('当前浏览器不支持直接打开文件');
    }
    try {
      const [handle] = await window.showOpenFilePicker({
        multiple: false,
        excludeAcceptAllOption: false,
        types: [JSON_FILE_TYPE],
      });
      if (!handle) return null;
      const file = await handle.getFile();
      this.importJson(await file.text(), {
        handle,
        fileName: handle.name || file.name || 'tower.json',
      });
      return this._project;
    } catch (error) {
      if (error?.name === 'AbortError') return null;
      throw error;
    }
  }
  async saveProjectFile(opts = {}) {
    const { saveAs = false } = opts;
    const suggestedName = this._fileName || 'tower.json';
    try {
      if (typeof window !== 'undefined' && 'showSaveFilePicker' in window) {
        let handle = saveAs ? null : this._fileHandle;
        if (!handle) {
          handle = await window.showSaveFilePicker({
            suggestedName,
            excludeAcceptAllOption: false,
            types: [JSON_FILE_TYPE],
          });
        }
        if (!handle) return null;
        const writable = await handle.createWritable();
        await writable.write(this.exportJson());
        await writable.close();
        this._fileHandle = handle;
        this._fileName = handle.name || suggestedName;
        return { kind: 'file', name: this._fileName };
      }
      return this.downloadJson(suggestedName);
    } catch (error) {
      if (error?.name === 'AbortError') return null;
      throw error;
    }
  }
}

function labelOf(st) {
  return STATUSES.find(s => s.key === st)?.name || st;
}

export const Store = new StoreImpl();
export const colorOf = (st) => STATUSES.find(s => s.key === st)?.color || '#888';
export const layerOf = (key) => LAYERS.find(l => l.key === key);
export const statusOf = (key) => STATUSES.find(s => s.key === key);
export { uuid };
